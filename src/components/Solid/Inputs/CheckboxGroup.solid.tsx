...
const CheckboxGroup = <T,>(props: CheckboxGroupProps<T>) => {
  const isChecked = (value: T) => {
    return props.selectedValues.some(v => v === value);
  };

  return (
    <>
      <div class={`modern-checkbox-group ${props.className || ""}`}>
          ...
          {(opt) => (
            <label class="modern-checkbox-label">
              <input
                ...
                onChange={(e) => props.onChange(opt.value, e.currentTarget.checked)}
                class="modern-checkbox-input"
              />
              <span class="modern-checkbox-custom"></span>
              <span class="modern-checkbox-text">{opt.label}</span>
            </label>
          )}
        </For>
      </div>
