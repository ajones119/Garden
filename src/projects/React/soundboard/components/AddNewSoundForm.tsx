...
const CHANNEL_OPTIONS: Option<string>[] = CHANNEL_LIST.map((channel) => ({
  value: channel,
  ...
  return (
    <form onSubmit={handleSave} className="bg-muted flex flex-col gap-2 p-2">
      ...
      <input
        id="url"
        type="text"
        className="w-full bg-muted border border-border border-solid color-foreground"
        value={currentConfig.url}
        onChange={(e) =>
          setCurrentConfig({ ...currentConfig, url: e?.target?.value })
        }
        placeholder="Enter URL..."
      />
      {urlError && (
        <p className="text-red-500 text-xs animate-slide-in">{urlError}</p>
      )}
      ...
      />
      {nameError && (
        <p className="text-red-500 text-xs animate-slide-in">{nameError}</p>
      )}
      <label htmlFor="channel" className="mt-4">
        Channel
      </label>
      <RadioButtons<string>
        id="channel"
        ...
        onChange={(value) =>
          setCurrentConfig({ ...currentConfig, channel: value })
        }
      />
