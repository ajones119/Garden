...
const PixelateAndDownloadImage = (
...
export const Pixelator = () => {
  ...
  return (
    <div class="w-full h-full p-1">
      <div
        id="controls"
        class="p-2 bg-background rounded-sm border-border border-1 border-solid mx-auto my-4 w-fit min-w-[250px] md:fixed md:bottom-2 md:right-2"
      >
        ...
        <Show
          ...
          keyed
          fallback={
            <BasicButton onClick={() => setImg(null)}>Reset File</BasicButton>
          }
        >
          {(image) => (
            <BasicButton
              onClick={() => PixelateAndDownloadImage(image, pixelSize())}
            >
              Download
            </BasicButton>
          )}
      ...
      <div class="h-[90%] w-full bg-muted border-border border-solid border">
        ...
        >
          {(imageValue) => (
            <PixelatorCanvas img={imageValue} pixelSize={pixelSize} />
          )}
        </Show>
      </div>
    </div>
